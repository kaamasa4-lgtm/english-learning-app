...
- [x] マイク権限の取得と録音開始・停止
- [x] `audio/webm` 形式で `Blob` を生成
- [x] `POST /upload-audio` でバックエンドへ送信
- [x] レスポンス JSON を画面に表示（エラーハンドリング含む）

### Phase 2: STT 結果の表示

- [x] 文字起こしテキストの表示
- [x] 単語ごとのタイムスタンプ・信頼度の一覧表示
